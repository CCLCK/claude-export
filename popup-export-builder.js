...
<\/script>`;
    const widgetOverrideStyle = `<style>
body .imap-cell:not(.hi):not(.hi2) {
...
<body>${widgetCode}${widgetOverrideStyle}${resizeScript}</body>
</html>`;
...
.runtime-debug-toggle {
  ...
  color: #7c4f10;
  box-shadow: 0 12px 24px rgba(20,20,19,.14);
  cursor: pointer;
  font: 600 13px/1 -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
}
.runtime-debug-toggle[hidden] { display: none; }
.runtime-debug-toggle svg {
  width: 16px;
  height: 16px;
}
.runtime-debug-toggle-badge {
  min-width: 20px;
  height: 20px;
  padding: 0 6px;
  border-radius: 999px;
...
.runtime-debug-panel {
  ...
  border-radius: 18px;
  background: rgba(255,255,255,.98);
  box-shadow: 0 18px 40px rgba(20,20,19,.18);
  overflow: hidden;
}
.runtime-debug-panel[hidden] { display: none; }
.runtime-debug-head {
  display: flex;
  align-items: center;
  justify-content: space-between;
...
@media print {
  ...
  .runtime-debug-toggle,
  .runtime-debug-panel { display: none !important; }
}
